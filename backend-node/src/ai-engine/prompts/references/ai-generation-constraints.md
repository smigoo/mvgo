...
**组件内有多个功能区块（header / tab / 图表区 / footer）时，禁止给内容区块写死固定高度。标准模式：固定功能条用固定高度，内容区块用 `flex: <flexGrow系数> 1 0; min-height: 0` 比例分配（flexGrow 为管线按 Figma 高度归一化的弹性系数，禁止把像素高度直接写进 grow）。**
...
.root-container {
  ...
  // ✅ 内容区块：flex-grow 用管线归一化系数（比例 = 设计稿高度比），引擎自动等比分配
  .stat-cards { flex: 0.67 1 0; min-height: 0; }   // 设计稿 110px → 系数 0.67（以下发值为准）
  .chart-area { flex: 1.33 1 0; min-height: 160px; } // 设计稿 220px → 系数 1.33（主图兜底 160）
...
```less
.chart-area {
  flex: 1.33 1 0;   // 设计稿高度 220px → 归一化系数 1.33（以下发值为准）
  min-height: 0;
  display: flex;
  flex-direction: column;

  // 内部多图表：grow 用归一化系数（比例 = 设计稿高度比 220:180:130）
  .chart-a { flex: 1.30 1 0; min-height: 160px; }  // 主图兜底
  .chart-b { flex: 1.06 1 0; min-height: 160px; }
  .chart-c { flex: 0.77 1 0; min-height: 100px; }  // 紧凑图（环形）兜底
}
```
...
| 图表/数据区 | `flex: <flexGrow系数> 1 0` + `min-height` 兜底 | 主图 160px / 紧凑图 100px |
| 底部状态栏 | 固定高度 + `flex-shrink: 0` | `height: 28px` |

**自检**：写完布局后，检查是否所有内容区块都是固定高度 — 如果是，改成 `flex: <flexGrow系数> 1 0` 比例分配。
