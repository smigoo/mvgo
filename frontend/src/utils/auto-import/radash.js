/*
 * @Description:Radash方法库（https://www.radash.wiki/getting-started.html）
 * @Date: 2024-09-01 14:47:23
 * @LastEditors: 潘强 panqiang1111
 * @LastEditTime: 2025-05-17 14:45:24
 * @Copyright: © 2024 Microvideo
 */

export const $radash = (function (exports) {
  'use strict'
  ...
  const replace = (list2, newItem, match) => {
    ...
    for (let idx = 0; idx < list2.length; idx++) {
      ...
      if (match(item, idx)) {
        return [...list2.slice(0, idx), newItem, ...list2.slice(idx + 1, list2.length)]
      }
    }
    return [...list2]
  }
  ...
  const series = (items, toKey = (item) => `${item}`) => {
    ...
    const spin = (current, num) => {
      ...
      return list(0, rel - 1).reduce((acc) => (num > 0 ? next(acc) : previous(acc)), current)
    }
    return {
      min,
      max,
