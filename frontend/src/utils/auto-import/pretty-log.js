...
const prettyLog = () => {
  const info = (textOrTitle, content = '') => {
    const title = $radash.isEmpty(content) ? 'Info' : textOrTitle
    const text = $radash.isEmpty(content) ? textOrTitle : content
    prettyPrint(title, text, '#909399')
  }

  const error = (textOrTitle, content = '') => {
    const title = $radash.isEmpty(content) ? 'Error' : textOrTitle
    const text = $radash.isEmpty(content) ? textOrTitle : content
    prettyPrint(title, text, '#F56C6C')
  }

  const warning = (textOrTitle, content = '') => {
    const title = $radash.isEmpty(content) ? 'Warning' : textOrTitle
    const text = $radash.isEmpty(content) ? textOrTitle : content
    prettyPrint(title, text, '#E6A23C')
  }

  const success = (textOrTitle, content = '') => {
    const title = $radash.isEmpty(content) ? 'Success' : textOrTitle
    const text = $radash.isEmpty(content) ? textOrTitle : content
...
// 只在开发环境中使用
export const log =
  $processEnv.VUE_APP_CURRENT_MODE === 'dev'
    ? prettyLog()
    : {
        info: () => {},
        error: () => {},
