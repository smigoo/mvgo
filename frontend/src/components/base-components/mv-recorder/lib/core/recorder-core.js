  ...
  var Connect = function (streamStore) {
    ...
    if (track && track.getSettings) {
      ...
      if (trackSR && trackSR != ctxSR) {
        tsMsg = $T(
          'eS8i::Stream的采样率{1}不等于{2}，将进行采样率转换（注意：音质不会变好甚至可能变差），主要在移动端未禁用回声消除时会产生此现象，浏览器有回声消除时可能只会返回16k采样率的音频数据，',
          0,
          trackSR,
          ctxSR
        )
    ...
    var oldScript = function () {
      ...
      _Disconn_n(stream)
      CLog(
        $T('7TU0::Connect采用老的{1}，', 0, scriptProcessor) +
          i18n.get(
            Recorder[ConnectEnableWorklet]
      ...
      var createNode = function () {
        ...
        CLog(
          $T(
            ...
            0,
            audioWorklet,
            RecTxt + '.' + ConnectEnableWorklet + '=false',
            scriptProcessor
          ) +
            webMTips +
            oldIsBest,
      ...
      try {
        ...
        CLog(
          $T(
            ...
            0,
            MRWebMPCM,
            RecTxt + '.' + ConnectEnableWebM + '=false',
            audioWorklet,
            scriptProcessor
          )
        )
...
*/
  Recorder.SampleData = function (pcmDatas, pcmSampleRate, newSampleRate, prevChunkInfo, option) {
    ...
    if (
      filter &&
      filter.fn &&
      ((filter.sr && filter.sr != pcmSampleRate) || (filter.srn && filter.srn != newSampleRate))
    ) {
      filter = null
    ...
    for (; index < nLen; index++) {
      ...
      if (step < 1) {
        ...
        for (var i0 = 0; i0 < il; i0++) {
          ...
          for (var j = 1; pos < end; pos++, j++) {
            ...
            else if (s < -0x8000) s = -0x8000 //Int16越界处理
      ...
      for (var i0 = 0, i2 = 0; i0 < il; i0++, i2++) {
        ...
        else if (val < -0x8000) val = -0x8000 //Int16越界处理
...
*/
  ...
  function initFn(set) {
    ...
    if ((vB && !IsNum(vB)) || (vS && !IsNum(vS))) {
      This.CLog(
        $T.G('IllegalArgs-1', [$T('VtS4::{1}和{2}必须是数值', 0, sampleRateTxt, bitRateTxt)]),
        1,
        set
      )
    }
  ...
  Recorder.prototype = initFn.prototype = {
    ...
    open: function (True, False) {
      ...
      if (!isBrowser) {
        failCall(
          $T.G('NonBrowser-1', ['open']) +
            $T('EMJq::，可尝试使用RecordApp解决方案') +
            '(' +
            GitUrl +
      ...
      var callUserMedia = function (retry) {
        ...
        if (trackSet[sampleRateTxt]) {
          This.CLog(
            $T(
              'IjL3::注意：已配置{1}参数，可能会出现浏览器不能正确选用麦克风、移动端无法启用回声消除等现象',
              0,
              atsTxt + '.' + sampleRateTxt
            ),
            3
          )
        }
        ...
        This.CLog(
          getUserMediaTxt +
            ...
            CtxState(ctx) +
            $T(
              'RiWe::，未配置 {1} 时浏览器可能会自动启用回声消除，移动端未禁用回声消除时可能会降低系统播放音量（关闭录音后可恢复）和仅提供16k采样率的音频流（不需要回声消除时可明确配置成禁用来获得48k高音质的流），请参阅文档中{2}配置',
              0,
              atsTxtJs,
              atsTxt
            ) +
    ...
    close: function (call) {
      ...
      if (This._O_ != Lock.O) {
        ...
        This.CLog(
          $T('hWVz::close被忽略（因为同时open了多个rec，只有最后一个会真正close）') + sTag,
          3
        )
        call()
        return
    ...
    envCheck: function (envInfo) {
      ...
      if (!errMsg) {
        ...
        if (set.takeoffEncodeChunk) {
          ...
          if (!hasFn) {
            errMsg =
              $T('2XBl::{1}类型不支持设置takeoffEncodeChunk', 0, type) +
              (This[type] ? '' : $T('LG7e::(未加载编码器)'))
        ...
        if (addTime > pcmTime / 5) {
          ...
          This.CLog(
            '[' +
              now +
              ']' +
              i18n.get(fixOpen ? $T('4Kfd::补偿{1}ms', 1) : $T('bM5i::未补偿{1}ms', 1), [addTime]),
            3
          )
    ...
    stop: function (True, False, autoClose) {
      ...
      var ok = function (blob, mime, duration) {
        ...
        if (dType == tABuf) {
          ...
          if (typeof Blob != 'function') {
            dErr =
              $T.G('NonBrowser-1', [dTag]) +
              $T('1skY::，请设置{1}', 0, RecTxt + '.' + tDDT + '="' + tABuf + '"')
          } else {
        ...
        if (dErr) {
          err(
            dErr != 1 ? dErr : $T('Vkbd::{1}编码器返回的不是{2}', 0, set.type, dType) + ', ' + dTag
          )
          return
        }
        if (set.takeoffEncodeChunk) {
      ...
      if (!This.isMock) {
        ...
        if (!This.state || isCtxWait) {
          err(
            $T('wf9t::未开始录音') +
              (isCtxWait ? $T('Dl2c::，开始录音前无用户交互导致AudioContext未运行') : '')
          )
          return
        }
      }
