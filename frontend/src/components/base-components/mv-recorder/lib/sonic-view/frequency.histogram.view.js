...
const install = (Recorder) => {
  ...
  非H5环境使用以上配置
    ...
    var canvas = null
    if (set[cCanvas]) {
      canvas = This.canvas = set[cCanvas]
    } else {
      ...
      canvas = This.canvas = thisElem.querySelector('canvas')
  ...
  fn.prototype = FrequencyHistogramView.prototype = {
    ...
    draw: function (frequencyData, sampleRate) {
      ...
      var stripeLinear1 =
        (set.stripeLinear && This.genLinear(ctx, set.stripeLinear, originY, originY - heightY)) ||
