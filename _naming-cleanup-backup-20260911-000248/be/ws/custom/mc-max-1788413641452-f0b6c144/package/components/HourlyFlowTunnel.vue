...
import bg5 from '../../resources/images/bg-3475.png'
...
const updateChart = () => {
  ...
  const option = {
    tooltip: {
      ...
      formatter: (params) => {
        let html = `<div style="font-size: calc(@fontSize * 0.8571); color: #333333;">${params[0].axisValue}时</div>`
        params.forEach(p => {
          html += `<div style="display: flex; align-items: center; gap: 4px; margin-top: 4px;">
            ...
            <span style="font-size: calc(@fontSize * 0.7143); color: #333333;">${p.seriesName}</span>
            <span style="font-size: @fontSize; font-weight: 500; color: #333333; margin-left: 8px;">${p.value}</span>
            <span style="font-size: calc(@fontSize * 0.7143); color: #333333;">辆</span>
          </div>`
        })
        return html
      }
