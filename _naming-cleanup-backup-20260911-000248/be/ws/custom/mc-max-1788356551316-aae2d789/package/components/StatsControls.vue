...
<img :src="icon2" class="c-env-monitor-toggle-icon" :class="{'c-env-monitor-toggle-icon-active': viewMode === 'chart' }" alt="图表视图" title="图表视图" @click="handleViewChange('chart')" />
<img :src="icon1" class="c-env-monitor-toggle-icon" :class="{'c-env-monitor-toggle-icon-active': viewMode === 'list' }" alt="列表视图" title="列表视图" @click="handleViewChange('list')" />
    </div>
  </div>
</template>
...
const props = defineProps({ currentValue: { type: Number, default: 40
  },
  thresholdInfo: {
    type: Object,
    default: () => ({
...
const handleViewChange = (mode) => { emit('view-change', mode)
}
</script>
