<template>
  <div class="c-mc-max-1787971283413-e8e268ae-c-monitor-flow-prediction">
    <!-- 区块标题 -->
    <div class="c-mc-max-1787971283413-e8e268ae-c-monitor-section-header">
      <div class="c-mc-max-1787971283413-e8e268ae-c-monitor-title-wrapper">
        <span class="c-mc-max-1787971283413-e8e268ae-c-monitor-title-icon"></span>
        <span class="c-mc-max-1787971283413-e8e268ae-c-monitor-title-text">流量预测</span>
      </div>
      <div class="c-mc-max-1787971283413-e8e268ae-c-monitor-header-controls">
        <!-- Tab 切换 -->
        <div class="c-mc-max-1787971283413-e8e268ae-c-monitor-location-tabs">
          <div
            v-for="tab in locationTabs"
            :key="tab.value"
            :class="['c-monitor-tab-item', { active: activeLocation === tab.value }]"
            @click="handleLocationChange(tab.value)"
           :style="activeLocation === tab.value ? { backgroundImage: 'url(' + bg4 + ')', backgroundSize: '100% 100%', backgroundRepeat: 'no-repeat' } : null">
            {{ tab.label }}
          </div>
        </div>
        <!-- 节假日预测按钮 -->
        <div class="c-mc-max-1787971283413-e8e268ae-c-monitor-holiday-btn" @click="handleHolidayPrediction">
          节假日预测
        </div>
      </div>
    </div>
    ...
    <!-- 图表容器 -->
    <div class="c-mc-max-1787971283413-e8e268ae-c-monitor-chart-body">
      <div ref="chartRef" class="c-mc-max-1787971283413-e8e268ae-c-monitor-chart-container"></div>
      <!-- 准确率标签 -->
      <div class="c-mc-max-1787971283413-e8e268ae-c-monitor-accuracy-labels">
        <span v-for="(acc, i) in accuracyData" :key="i" class="c-mc-max-1787971283413-e8e268ae-c-monitor-accuracy-item">
          {{ acc }}
        </span>
      </div>
    </div>
