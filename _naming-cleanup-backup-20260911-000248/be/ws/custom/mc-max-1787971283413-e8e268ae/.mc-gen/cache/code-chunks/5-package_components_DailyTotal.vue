  ...
  <div class="c-monitor-daily-total">
    ...
    <div class="c-monitor-daily-header">
      ...
      <a-select v-model:value="timeRange" class="c-monitor-time-selector">
        ...
        <a-select-option value="7d">本周</a-select-option>
        <a-select-option value="30d">本月</a-select-option>
      </a-select>
    </div>

    <!-- 数据卡片区：背景图 + 3个统计项 -->
    <div class="c-monitor-daily-body" :style="{ backgroundImage: `url(${bg1})` }">
      <!-- 左侧：江阴靖江长江隧道 -->
      <div class="c-monitor-stat-card">
        <div class="c-monitor-stat-label">江阴靖江长江隧道</div>
        <div class="c-monitor-stat-value">34,620</div>
      </div>

      <!-- 中间：装饰图标（地图图标） -->
      <div class="c-monitor-decoration-icon">
        <img :src="bgm" alt="decoration" class="c-monitor-decoration-img" />
      </div>

      <!-- 右侧：江阴大桥 -->
      <div class="c-monitor-stat-card">
        <div class="c-monitor-stat-label">江阴大桥</div>
        <div class="c-monitor-stat-value c-monitor-stat-value-secondary">82,379</div>
      </div>
    </div>
  </div>
</template>
...
.c-monitor-stat-value-secondary {
  color: #0c9dbe;
}

.c-monitor-decoration-icon {
  position: absolute;
  left: 50%;
  top: 50%;
