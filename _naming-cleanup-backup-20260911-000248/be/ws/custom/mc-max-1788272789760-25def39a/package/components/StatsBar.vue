<template>
  <div class="c-monitor-stats-bar">
    ...
    >
      ...
      <img :src="card.icon" class="c-monitor-stats-card-icon" :alt="card.titleLines.join('')"  :style="{ backgroundImage: 'url(' + bg13 + ')', backgroundSize: '100% 100%', backgroundPosition: '0px 0px', backgroundRepeat: 'no-repeat' }"/>
      <!-- 卡片文字区：标题 + 总数行 + 异常数行（纵向堆叠） -->
      <div class="c-monitor-stats-card-body" :style="{ backgroundImage: 'url(' + bg14 + ')', backgroundSize: '100% 100%', backgroundPosition: '0px 0px', backgroundRepeat: 'no-repeat' }">
        <div class="c-monitor-stats-card-title" :style="{ backgroundImage: 'url(' + bg9 + ')', backgroundSize: '100% 100%', backgroundPosition: '0px 0px', backgroundRepeat: 'no-repeat' }">
          <span
            v-for="(line, idx) in card.titleLines"
            :key="idx"
            class="c-monitor-stats-card-title-line"
           :style="{ backgroundImage: 'url(' + bg3 + ')', backgroundSize: '100% 100%', backgroundPosition: '0px 0px', backgroundRepeat: 'no-repeat' }">{{ line }}</span>
        </div>
        <div class="c-monitor-stats-card-line" :style="{ backgroundImage: 'url(' + bg10 + ')', backgroundSize: '100% 100%', backgroundPosition: '0px 0px', backgroundRepeat: 'no-repeat' }">
          <span class="c-monitor-stats-card-label" :style="{ backgroundImage: 'url(' + bg4 + ')', backgroundSize: '100% 100%', backgroundPosition: '0px 0px', backgroundRepeat: 'no-repeat' }">总数:</span>
          <span class="c-monitor-stats-card-value c-monitor-stats-card-value--total" :style="{ backgroundImage: 'url(' + bg5 + ')', backgroundSize: '100% 100%', backgroundPosition: '0px 0px', backgroundRepeat: 'no-repeat' }">{{ card.total }}</span>
        </div>
        <div class="c-monitor-stats-card-line">
          <span class="c-monitor-stats-card-label">异常数:</span>
          <span class="c-monitor-stats-card-value c-monitor-stats-card-value--abnormal" :style="{ backgroundImage: 'url(' + bg6 + ')', backgroundSize: '100% 100%', backgroundPosition: '0px 0px', backgroundRepeat: 'no-repeat' }">{{ card.abnormal }}</span>
        </div>
      </div>
    </div>
  </div>
...
<script setup>
import bg14 from '../../resources/images/bg-8759.png'
import bg13 from '../../resources/images/bg-8730.png'
import bg10 from '../../resources/images/bg-8643.png'
import bg9 from '../../resources/images/bg-8614.png'
...
.c-monitor-stats-card-value {
  ...
  font-size: calc(var(--fontSize, 14px) * 1.43);
  font-weight: 700;
  line-height: 1.2;
}
