  ...
  <div class="c-monitor-flow-prediction">
    ...
    <div class="c-monitor-section-header">
      <div class="c-monitor-section-title-group">
        <img :src="icon3" class="c-monitor-section-icon" alt="icon" />
      ...
      <div class="c-monitor-location-tabs">
        <span
          ...
          :key="location.key"
          :class="['c-monitor-location-tab', { active: activeLocation === location.key }]"
          @click="handleLocationChange(location.key)"
        >
          {{ location.label }}
        </span>
      </div>
      ...
      <!-- 节假日预测链接 -->
      <a href="#" class="c-monitor-holiday-link" @click.prevent="handleHolidayClick">
        节假日预测&gt;
      </a>
    </div>
    ...
    <div class="c-monitor-chart-body">
      ...
      <div class="c-monitor-chart-legend">
        ...
        <span
          class="c-monitor-legend-item"
          :class="{ active: legendState.predicted }"
...
// 地点选项
const locations = ref([
  { label: '江阴靖江长江隧道', key: 'tunnel' },
  { label: '江阴大桥', key: 'bridge' }
])
...
const dataMap = {
  tunnel: {
    actual: [2800, 2900, 3100, 3000, 2950],
    predicted: [2850, 2920, 3050, 2980, 2900]
  },
  bridge: {
    actual: [3200, 3300, 3400, 3350, 3300],
    predicted: [3250, 3320, 3380, 3330, 3280]
  }
}
...
const handleLocationChange = (key) => {
  if (activeLocation.value === key) return
