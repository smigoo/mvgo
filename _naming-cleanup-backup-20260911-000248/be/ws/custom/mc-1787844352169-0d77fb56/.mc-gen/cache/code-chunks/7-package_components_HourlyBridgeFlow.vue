<template>
  <div class="c-monitor-hourly-bridge">
    ...
    <div class="c-monitor-section-title">
      <span class="c-monitor-title-dot"></span>
    ...
    <div class="c-monitor-chart-legend">
      ...
      >
        <i class="c-monitor-legend-dot c-monitor-legend-dot-primary"></i>
      ...
      >
        <i class="c-monitor-legend-dot c-monitor-legend-dot-warning"></i>
...
const mockData = {
  ...
  beijing: [150, 180, 220, 450, 820, 1200, 1850, 2100, 825, 1650, 1100, 600, 300],
  shanghai: [120, 160, 200, 420, 780, 1100, 1750, 1980, 831, 1580, 1020, 550, 280]
}
...
1. **布局结构**：
   - 区块标题：左侧装饰条 + 文字
   - 图例：两个可点击的图例项（北京方向/上海方向）
   - 图表：echarts 柱状图容器

2. **图表功能**：
   - 使用 echarts.init() 初始化
   - ResizeObserver 监听容器尺寸变化
   - watch(chartRef) 处理 base-panel 重建场景
   - 图例点击触发 dispatchAction 实现系列切换

3. **样式要点**：
   - flex 高度分配：`flex: 180 1 0`（按设计稿比例）
   - 最小高度：`min-height: 160px`（主图兜底）
   - 图例激活态：opacity 切换
   - 柱状图：8px 宽度，圆角顶部
