...
const getChartOption = () => {
  return {
    tooltip: {
      ...
      formatter: (params) => {
        let html = `<div style="font-size:12px;color:#333;font-weight:400;margin-bottom:4px">${params[0].name}时</div>`
        params.forEach((p) => {
          ...
          html += `<div style="display:flex;align-items:center;gap:4px;margin-bottom:2px">
            ...
            <span style="color:#333;font-size:9.6px">${p.seriesName}</span>
            <span style="font-size:14px;font-weight:500;color:#333;margin-left:auto">${p.value}</span>
            <span style="font-size:9.6px;color:#333">辆</span>
          </div>`
        })
        return html
      }
