...
<div v-for="item in navItems" :key="item.value" :class="['c-monitor-nav-item', { active: activeCategory === item.value }]" @click="handleCategoryClick(item.value)" >
...
<div v-for="device in deviceList" :key="device.id" class="c-monitor-device-card" @click="handleDeviceClick(device)" >
...
<script setup>
const props = defineProps({ navItems: { type: Array, default: () => []
  },
  deviceList: {
    type: Array,
    default: () => []
...
const handleCategoryClick = (value) => { emit('category-change', value)
}

const handleDeviceClick = (device) => { emit('device-click', device)
}
</script>
