<template>
  <base-panel class="c-mc-max-1788349399134-9c3aff7f" panelKey="default-panel">
      <template #header_right>
        <!-- 由生成器按 Figma headerSlots 确定性补齐（模型漏生成兜底） -->
        <span style="display:inline-flex;align-items:center;gap:4px;margin-left:16px;font-size: calc(@fontSize * 0.9286);line-height:1;white-space:nowrap;">柱状图切换图标（蓝色，当前激活）</span>
        <span style="display:inline-flex;align-items:center;gap:4px;margin-left:16px;font-size: calc(@fontSize * 0.9286);line-height:1;white-space:nowrap;">列表/表格切换图标（带橙色角标数字6）</span>
      </template>
    <div class="c-env-monitor-root">
      <!-- Tab切换区 -->
      <div class="c-env-monitor-tabs-container" :style="{ backgroundImage: 'url(' + bg1 + ')', backgroundSize: '100% 100%', backgroundPosition: '0px 0px', backgroundRepeat: 'no-repeat' }"><img :src="icon2" class="auto-mounted-icon" style="max-width:48px;max-height:48px;object-fit:contain" alt="icon" /><img :src="icon1" class="auto-mounted-icon" style="max-width:48px;max-height:48px;object-fit:contain" alt="icon" />
        <div 
          v-for="tab in tabs" 
          :key="tab.value"
