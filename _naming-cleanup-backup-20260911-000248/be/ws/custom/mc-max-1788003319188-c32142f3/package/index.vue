<template>
  <base-panel panelKey="default-panel" :style="{ backgroundImage: 'url(' + bg5 + ')', backgroundSize: '100% 100%', backgroundRepeat: 'no-repeat' }">
    <div class="c-mc-max-1788003319188-c32142f3-c-monitor-root">
      <!-- 当日总流量 -->
      <DailyTotalFlow 
        :tunnel-traffic="tunnelTraffic.value"
        :bridge-traffic="bridgeTraffic.value"
      />

      <!-- 江阴靖江长江隧道24小时流量 -->
      <TunnelHourlyChart 
        :chart-data="tunnelHourlyData"
      />

      <!-- 江阴大桥24小时流量 -->
      <BridgeHourlyChart 
        :chart-data="bridgeHourlyData"
      />
...
// onUnmounted: 清理资源（子组件各自清理图表）

onUnmounted(() => {
  // 主组件无需额外清理，子组件自行处理 echarts dispose
})

// === 事件处理函数 ===

// 流量预测地点切换（江阴靖江长江隧道 <-> 江阴大桥）

const handleLocationChange = (location) => {
  activeLocation.value = location
  // 子组件 FlowPrediction 会根据 activeLocation 更新图表数据
}
</script>
