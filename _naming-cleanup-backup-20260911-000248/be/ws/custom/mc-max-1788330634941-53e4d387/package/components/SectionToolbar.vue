<template>
  <div class="c-env-monitor-toolbar">
    <!-- 左侧 Tab 切换栏 -->
    <div class="c-env-monitor-tabs" :style="{ backgroundImage: 'url(' + bg1 + ')', backgroundSize: '100% 100%', backgroundPosition: '0px 0px', backgroundRepeat: 'no-repeat' }">
      <div
        v-for="tab in tabs"
        :key="tab.value"
...
const tabs = [{ label: '一氧化碳', value: '一氧化碳' }, { label: '能见度', value: '能见度' }, { label: '洞内照明', value: '洞内照明' }, { label: '洞外光强', value: '洞外光强' }]
...
.c-env-monitor-tab-text {
  font-size: var(--fontSize, 14px); /* 14px */;
  line-height: 1;
  color: #2c9bea;
  user-select: none;
}
...
.c-env-monitor-badge {
  ...
  background: #f53f3f;
  color: #ffffff;
  font-size: calc(var(--fontSize, 14px) * 0.857); /* ≈ 12px */;
  line-height: 1;
  border-radius: 50%;
  box-sizing: border-box;
}
