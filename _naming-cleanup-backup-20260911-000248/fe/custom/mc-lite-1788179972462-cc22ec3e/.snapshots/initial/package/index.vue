...
const initChartWhenReady = () => {
  ...
  resizeObserver = new ResizeObserver(() => {
    if (container.clientWidth > 0 && container.clientHeight > 0 && !chart) {
...
// 🛡️ watch 驱动初始化（沙箱/预览环境下 onMounted 触发时 ref 可能尚未填充，
// 原 onMounted 一次性判断 chartRef.value 为 null 即 return → 永不 init → 图表空白）。
// watch immediate 在 ref 就绪后立即触发；onMounted 作兜底重试，幂等。
watch(chartRef, (el) => {
  if (el && !chart) nextTick(() => initChartWhenReady())
}, { immediate: true })
onMounted(() => {
