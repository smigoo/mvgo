<template>
  <div class="mc-robot-modal" :style="cssVars">
    ...
    <div class="mc-body">
      <div class="mc-grid">
        <div v-for="(item, index) in safeItems" :key="index" class="mc-info-row">
          <span class="mc-label">{{ item.label }}</span>
          <span class="mc-value" :class="`text-${item.colorType || 'default'}`">{{ item.value }}</span>
        </div>
      </div>
    </div>
  </div>
...
// 安全获取 items，防止传入 null/非数组时 v-for 报错
const safeItems = computed(() => {
  return Array.isArray(props.items) ? props.items : [];
});

// CSS 变量计算，支持主题切换（增加 null 安全兜底）
const cssVars = computed(() => {
  const theme = props.theme || {};
  return {
    '--mc-bg-color': theme.backgroundColor || '#00222b',
    '--mc-text-color': theme.textColor || '#ffffff',
    '--mc-primary-color': theme.primaryColor || '#00ffff',
    '--mc-success-color': theme.successColor || '#00ff00',
    '--mc-danger-color': theme.dangerColor || '#ff0000',
    '--mc-label-color': theme.labelColor || '#e0e0e0',
  };
});
</script>
