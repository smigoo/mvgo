...
<div v-for="device in devices" :key="device.id" class="c-monitor-device-card" @click="handleCardClick(device)" >
...
const props = defineProps({ activeTab: { type: String, default: '监控'
  },
  devices: {
    type: Array,
    default: () => []
...
const handleCardClick = (device) => { emit('card-click', device)
}
</script>
...
.c-monitor-device-online {
  font-size: calc(var(--fontSize, 14px) * 1.429);
  font-weight: bold;
  color: #1890ff;
  line-height: 1.2;
}
