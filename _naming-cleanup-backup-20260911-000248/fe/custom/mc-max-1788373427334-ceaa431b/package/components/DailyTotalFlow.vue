<template>
  <div class="c-monitor-daily-total-flow">
    <!-- 区块标题行 -->
    <div class="c-monitor-daily-header"><img :src="icon2" class="auto-mounted-icon" style="width:18px;height:18px;object-fit:contain" alt="icon" />
      <div class="c-monitor-daily-title"><img :src="icon3" class="auto-mounted-icon" style="width:18px;height:18px;object-fit:contain" alt="icon" />
      ...
      <div class="c-monitor-daily-time-selector" @click="toggleDropdown">
        ...
        <div class="c-monitor-daily-dropdown" v-show="dropdownOpen">
          <div
           ...
           :style="selectedTime === opt.label ? { backgroundImage: 'url(' + bg3 + ')', backgroundSize: '100% 100%', backgroundRepeat: 'no-repeat' } : null">
            {{ opt.label }}
          </div>
        </div>
      </div>
...
.c-monitor-daily-time-arrow {
  font-size: calc(var(--fontSize, 14px) * 0.57);
  color: #a8abb2;
