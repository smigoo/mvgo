<template>
  <base-panel panelKey="default-panel">
    ...
    <template #header_right>
      <div class="c-mc-max-1787973616111-7b5cb9e2-c-monitor-location-tabs">
        <div
          v-for="location in locations"
          :key="location.value"
          :class="['c-monitor-location-tab', { active: activeLocation === location.value }]"
          @click="activeLocation = location.value"
         :style="activeLocation === location.value ? { backgroundImage: 'url(' + bg3 + ')', backgroundSize: '100% 100%', backgroundRepeat: 'no-repeat' } : null">
          {{ location.label }}
        </div>
      </div>
    </template>
    ...
    <!-- 主内容区 -->
    <div class="c-mc-max-1787973616111-7b5cb9e2-c-monitor-root">
      <!-- 当日总流量区域 -->
      <DailyTotalSection
...
// 生命周期钩子
onMounted(() => {
  // 触发组件加载完成事件（businessEvents中声明的onload事件）
  runtimeBuilder.publishEvent('monitor-onload', {
    componentId: 'monitor',
    timestamp: Date.now()
  })
