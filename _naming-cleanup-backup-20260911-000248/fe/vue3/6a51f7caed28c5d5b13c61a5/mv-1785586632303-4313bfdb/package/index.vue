...
import flowApi from './api/flow.js'  // slot-3: card (统计数据)
import { mockTabs } from './api/stub-slot-1.js'  // stub slot-1
import { mockCards } from './api/stub-slot-2.js'  // stub slot-2
// === END API imports ===

const card = ref({})  // slot-3: card (统计数据)

// === API 对接调用 ===
onMounted(async () => {
  // slot-3: card (统计数据) → flow.getTodayFlow
  try {
    const __res_slot_3 = await flowApi.getTodayFlow()
    if (__res_slot_3?.code === 200 || __res_slot_3?.data) {
      card.value = __res_slot_3.data || __res_slot_3
    }
  ...
  // stub slot-2: cards — 使用 mock data
