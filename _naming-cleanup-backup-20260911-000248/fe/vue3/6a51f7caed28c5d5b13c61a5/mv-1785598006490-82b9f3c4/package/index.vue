            ...
            <div class="vehicle-card-content">
              ...
              <div class="vehicle-stats">
                <div class="vehicle-stat-item">
                  ...
                  <span class="vehicle-type-value客车">22350</span>
                </div>
                <div class="vehicle-stat-item">
                  <span class="vehicle-type-label">货车</span>
                  <span class="vehicle-type-value货车">16270</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="vehicle-card" :style="{ backgroundImage: `url(${bgm_4})` }">
            ...
            <div class="vehicle-card-content">
              ...
              <div class="vehicle-stats">
                <div class="vehicle-stat-item">
                  <span class="vehicle-type-label">客车</span>
                  <span class="vehicle-type-value客车">66109</span>
                </div>
                <div class="vehicle-stat-item">
                  <span class="vehicle-type-label">货车</span>
                  <span class="vehicle-type-value货车">16270</span>
...
import flowApi from './api/flow.js'  // slot-3: option (图表数据)
import { mockTimeOptions } from './api/stub-slot-1.js'  // stub slot-1
import { mockForecastTabs } from './api/stub-slot-2.js'  // stub slot-2
// === END API imports ===

const option = ref({})  // slot-3: option (图表数据)
const timeOptions = ref([])  // slot-1: time (列表数据)
const forecastTabs = ref([])  // slot-2: forecast tabs (列表数据)

// === API 对接调用 ===
onMounted(async () => {
  // slot-3: option (图表数据) → flow.getTodayFlow
  try {
    const __res_slot_3 = await flowApi.getTodayFlow()
    if (__res_slot_3?.code === 200 || __res_slot_3?.data) {
      option.value = __res_slot_3.data || __res_slot_3
    }
  } catch (e) { console.error('slot-3 flowApi.getTodayFlow():', e) }

  // stub slot-1: timeOptions — 使用 mock data
  timeOptions.value = mockTimeOptions
  // TODO: 接口就绪后替换为真实调用

  // stub slot-2: forecastTabs — 使用 mock data
  forecastTabs.value = mockForecastTabs
  // TODO: 接口就绪后替换为真实调用
